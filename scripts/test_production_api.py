#!/usr/bin/env python3
"""
CityResQ360 - Production API Test
Test all endpoints on production server: https://api.cityresq360.io.vn
"""
...
class PostmanCollectionTester:
    def __init__(self, collection_path: str, base_url: str, image_path: str = None):
        ...
        self.skip_endpoints = [
            'Register',  # Don't create accounts on production
            'Login',  # Already logged in once
            'Forgot Password',
            'Reset Password',
            'Logout',  # Don't logout during testing
            'Refresh Token',
            # 'Upload Media',  # Now enabled with logo.png
            # 'Create Report',  # Don't spam production
            # 'Create Comment',  # Don't spam production
            'Delete Report',
            'Delete Comment',
            'Delete Media',
            'Delete Notification',
    ...
    def test_endpoint(self, folder: str, request_item: Dict) -> APITestResult:
        ...
        if name in self.skip_endpoints:
            return APITestResult(
                ...
                error_message="Skipped for production safety"
            )
        
        # Skip POST/PUT/DELETE except safe ones
        if method in ['POST', 'PUT', 'DELETE']:
            # Only allow safe read-only-like operations
            safe_operations = [
                'Vote Report', 'Like Comment', 'Unlike Comment',
                'Mark as Read', 'Mark All as Read', 'Update Profile',
                'Update FCM Token', 'Verify Email', 'Verify Phone',
                'Increment View', 'Rate Report', 'Redeem Reward',
                'Update Notification Settings', 'Sync Weather Data',
                # Production test operations
                'Upload Media', 'Create Report', 'Create Comment',
            ]
            if name not in safe_operations:
                return APITestResult(
                    ...
                    method=method,
                    status=TestStatus.SKIPPED,
                    error_message="Write operation skipped on production"
                )
            ...
            try:
                ...
                with open(self.image_path, 'rb') as f:
                    ...
                    data = {
                        ...
                        'mo_ta': 'Test upload from production automation'
                    }
        ...
        try:
            ...
            response = self.session.request(
                ...
                timeout=15  # Longer timeout for production
            )
...
def main():
    ...
    IMAGE_PATH = "/Volumes/MyVolume/Laravel/CityResQ360-DTUDZ/static/img/muctieu.png"
    
    # Use production credentials
    # TODO: Update with actual production test account
    EMAIL = "nguyenvanan@gmail.com"
    PASSWORD = "password123"
    
    OUTPUT_FILE = "production_api_test_report.txt"
    
    print(f"""
{'='*80}
⚠️  PRODUCTION API TESTING
{'='*80}

WARNING: Testing production server!
- Write operations (POST/PUT/DELETE) are mostly SKIPPED
- Only safe read operations (GET) will be tested
- Upload Media: ENABLED with muctieu.png
- Using test account: {EMAIL}

Press Ctrl+C to cancel...
{'='*80}
""")
    
    # Create tester with image path
    tester = PostmanCollectionTester(COLLECTION_PATH, BASE_URL, IMAGE_PATH)
    
    # Login
    if not tester.login(EMAIL, PASSWORD):
        print("\n❌ Production login failed. Cannot proceed.")
    ...
    if failed_count > 0 or error_count > 0:
        print(f"\n⚠️  Found {failed_count} failures and {error_count} errors")
        return 1
